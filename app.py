#importando a biblioteca flet
#precisa instalar o flet com pip install flet no cmd
...
class Mensagem:
    ...
    def __del__(self):
        return f"{self.usuario} destuído com sucesso."
...
def main(page: ft.Page):
    ...
    def entrar(e):
        if not nome_usuario.value:
            ...
            page.pubsub.send_all(Mensagem(usuario=nome_usuario.value,texto=f"{nome_usuario.value} entrou no chat.", tipo_msg="login_messege"))
            page.update()     
    ...
    page.add(
        ft.Row(
            [ft.Text("Oh Chat!",size= 60,width="bold")],
